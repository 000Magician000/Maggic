...
{
    public class IceFusionSphere : ModProjectile
    {
            ...
            for (int i = 0; i < 5; i++)
            {
                Dust d = Dust.NewDustDirect(
                    new Vector2(projectile.Center.X + Main.rand.NextFloat(-10, 10),
                        projectile.Center.Y + Main.rand.NextFloat(-10, 10)), 0, 0, 156);
                d.noGravity = true;
            }
        }
        ...
        private void Explode(int count)
        {
            ...
            for (int i = -count / 2; i < count / 2; i++)
            {
                ...
                Projectile p = Projectile.NewProjectileDirect(projectile.Center, vel,
                    mod.ProjectileType("IceFusionSphere2"),
    ...
    public class IceFusionSphere2 : ModProjectile
    {
        ...
        public override bool PreKill(int timeLeft)
        {
            ...
            DustHelper.CreateDust(new Vector2(projectile.Center.X + Main.rand.NextFloat(-4,4),
                projectile.Center.Y + Main.rand.NextFloat(-4, 4)), 156, Color.White,projectile.oldVelocity.ToRotation().ToRotationVector2()*Main.rand.NextFloat(2,6),1,0,1.4f,true);
